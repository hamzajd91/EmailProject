...
// const allAds = [
//   {link: "/profile/2863612", image: ad5},
//   {link: "/profile/1099866", image: ad4},
//   {link: "/subscription2/free", image: ad1},
//   {link: "/subscription2/free", image: ad3},
//   {link: "/subscription2/free", image: ad2},
// ];
...
class CompaniesList extends Component<Props, State> {
  ...
  async componentDidMount() {
    ...
    //   this.setState({
    //     adNum: Math.floor(Math.random() * 2 + 1),
    //   });
  }
  ...
  handlePageChange = (pageNumber: number) => {
    ...
    this.props.history.push({
      pathname: "/search",
      search: `?q=${_query.q}&location=${_query.location}&page=${pageNumber}`,
    });
  };
  ...
  render() {
    ...
    const {companyResponse, companiesData, loading, activePage, setShowProfile, advertisementsCompanies} = this.props;
          ...
          <Container className="mt-3">
            {total !== undefined && (
              <div className="no_of_record">
                Showing {lowerCompanyNumber}-{upperCompanyNumber} of {total} results
              </div>
            )}
          </Container>
          <Col className="companies_list">
            {/* <Col sm={12} md={6} lg={6}>
              {total !== undefined && (
                <div className="sortFilters_wrapper">
                  <select defaultValue={sort} onChange={this.handleSortFilter} className="form-control sortFilters">
                    {sortFilters !== undefined &&
                      sortFilters.map((filter: any, i) => (
                        <option key={i.toString()} value={filter.value}>
                          {filter.display}
                        </option>
                      ))}
                  </select>
            ...
            {/* <Container>
              <div className="row justify-content-center">
                {advertisementsCompanies.data.map((add: any, index: number) => {
                  if (add.bannerImage) {
                    return (
                      <div className="col-md-4 advertisement_gird" key={index}>
                        <Link to={`/profile/${add.company.id}`}>
                          <img className="ad-image-search" src={add.bannerImage} alt="hindsyght ad" srcSet="" />
                        </Link>
                      </div>
                    );
                  }
            ...
            {sponsoredResults.length > 0 && (
              <Row>
                <div className="divider_all">
                  <Container>Sponsored Results</Container>
                </div>
              </Row>
            )}
            ...
            <div className="row">
              {sponsoredResults.map((company, index) => (
                <React.Fragment key={index.toString()}>
                  <div className="col-12 company_grid sponsered" tabIndex={index} onClick={() => setShowProfile(true)}>
                    <CompanyItem
                      handleRequestProposal={this.handleRequestProposal}
            ...
            {allResults.length > 0 && (
              <Row>
                <div className="divider_all">
                  <Container>All Results</Container>
                </div>
              </Row>
            )}
            ...
            <div className="row">
              {allResults.map((company, index) => (
                <React.Fragment key={company.id}>
                  <div className="col-12 company_grid" tabIndex={index} onClick={() => setShowProfile(true)}>
                    <CompanyItem handleRequestProposal={this.handleRequestProposal} company={company} />
                  </div>
                </React.Fragment>
              ))}
            </div>
