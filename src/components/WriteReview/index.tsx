...
function WriteReview(props: any) {
  ...
  const [allAddress, setAllAddress]: any = React.useState([]);
  ...
  const [addressError, setAddressError]: any = React.useState(null);
  ...
  const nextStep = () => {
    ...
    if (radioValue == null || radioValue.value == "") {
      ...
      let nextStep = (parseFloat(activeStep) + 1).toString();
  ...
  const postReview = async () => {
    ...
    if (address.length == 0) {
      return setAddressError("Please select location.");
    }
    setTermsErr("");
    setAddressError(null);
    setIsPosting(true);
    ...
    formData.reviewId = new URLSearchParams(props.location.search).get("review");

    try {
      await appApi.post(`/companies/${props.match.params.id}/review`, formData);
      return props.history.push(`/profile/${props.match.params.id}`);
    } catch (error) {
      setIsPosting(false);
    }
  };
  ...
  function addReviewCount() {
    const reviewId = new URLSearchParams(props.location.search).get("review");
    if (reviewId) {
      appApi.get(`users/review/click/${reviewId}`);
    }
  }
  ...
  useEffect(() => {
    const getCompanyInfo = async () => {
      ...
      const companyResult: any = await appApi.get(`companies/${props.match.params.id}/branches`);

      if (companyResult.data.canEdit) {
        setCanAddReview(false);
        return setIsLoading(false);
      }
      addReviewCount();
      setCompanyData(companyResult.data);
      setAddress([companyResult.data.id]);
      setAllAddress(companyResult.data.branches);
      setIsLoading(false);
            ...
            <div className="row">
              ...
              <div className="col-md-6">
                <div className="anonymous_Review_Block">
                  ...
                  <FormControl error={addressError ? true : false} style={{ width: "100%" }}>
                    <Select
                      MenuProps={{
                        ...
                        return (
                          <div className={classes.chips}>
                            {selected.map((value: any) => {
                              const _add = allAddress.find((address: any) => address.id == value);
                              return (
                                <Chip
                                  className={classes.chip}
                                  key={value}
                                  label={`${_add.address1} ${_add.address2}, ${_add.city}, ${_add.state}`}
                                />
                              );
                            })}
                          </div>
                    ...
                    >
                      {allAddress.map((data: any, index: any) => {
                        // const checked = !!currentFilters.industries.some((d: any) => d === industry.name);
                        return (
                          <MenuItem value={data.id} key={index}>
                            <Checkbox checked={address.includes(data.id)} />
                            <ListItemText primary={`${data.address1} ${data.address2}, ${data.city}, ${data.state}`} />
                          </MenuItem>
                        );
                      })}
                    </Select>
                    {addressError ? <FormHelperText error>{addressError}</FormHelperText> : null}
                  </FormControl>
                </div>
              </div>
            </div>
          ...
          <div className="container">
            <div className="row justify-content-center">
              <div className="col-md-8 text-center">
                ...
                <h5 className="mt-4">"You can not write a review for your claimed company."</h5>
                <br />
                <Link className="compny_btn" to={`/profile/${props.match.params.id}`}>
